...
describe("SW-003 workflow contract", () => {
  ...
  it("classifies shared manual-mapping examples with Zod", async () => {
    const path = new URL("../examples/confirmed-mappings/3.0.0.json", import.meta.url);
  ...
  it("classifies website metadata as the bounded domain family", () => {
    expect(inferSemanticFamily({ mappingId: "site", label: "Website URL", aColumn: "website", bColumn: "homepage", normalizer: "text" })).toBe("domain");
  });
  ...
  it("validates complete candidate evidence independently from queue summaries", () => {
    ...
    const detail = CandidateEvidenceDetailSchema.parse({ contractVersion: "1.0.0", runId: "run-1", candidate, alternatives: [candidateSummary], reviewState: "needs_review", deferred: false, collisionARowIds: [], effectiveCollisionARowIds: [], humanDecision: null, conflicts: [], matcherVersion: "explainable-matcher-v0.3.0", candidateEngineVersion: "candidate-engine-v0.4.0" });
