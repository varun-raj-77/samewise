  ...
  return <div className="app-shell">
    <aside className="app-sidebar">
      ...
      <nav className="app-navigation" aria-label="Samewise navigation">
        <div className="product-nav">
          ...
          <button aria-current={screen === "evaluation" ? "page" : undefined} onClick={() => setScreen("evaluation")}><span aria-hidden="true">E</span>Evaluation</button>
        </div>
    ...
    <main className={screen === "evaluation" ? "workspace evaluation-layout" : "workspace"}>
      <section className={screen === "review" ? "content review-content" : "content"}>
        ...
        {screen === "export" && run?.summary && <section aria-labelledby="export-title"><p className="eyebrow">Step 7 · Reproducible artifacts</p><h1 id="export-title">Report everything. Trust only what is ready.</h1><p className="lede">The reconciliation report preserves uncertainty. Trusted merged output is gated until identity review and every relevant field conflict are resolved. The manifest records the versions and hashes that produced this snapshot.</p><div className="export-run-context"><span><small>Run</small><code>{run.runId}</code></span><span><small>Dataset A</small><code>{run.datasets.A?.sha256.slice(0, 12)}…</code></span><span><small>Dataset B</small><code>{run.datasets.B?.sha256.slice(0, 12)}…</code></span></div><div className="export-artifact-list"><article className="export-artifact"><div><small>Reconciliation report</small><strong>Available</strong><code>{RECONCILIATION_EXPORT_VERSION}</code><p>Includes automatic and human identity outcomes, deferred or pending work, source-only records, and field-resolution state.</p></div><button className="secondary" onClick={() => void downloadExport("reconciliation")} disabled={busy}>Download reconciliation report</button></article><article className="export-artifact"><div><small>Trusted merged output</small><strong>{run.trustedExportReadiness.ready ? "Ready" : "Blocked"}</strong><code>{TRUSTED_EXPORT_VERSION}</code><p>Contains only confirmed identities and resolved required field state. KEEP BOTH remains explicit.</p></div><button className="primary" onClick={() => void downloadExport("trusted")} disabled={busy || !run.trustedExportReadiness.ready}>Download trusted merged output</button></article><article className="export-artifact"><div><small>Run provenance manifest</small><strong>Available</strong><code>{RUN_MANIFEST_VERSION}</code><p>Records source fingerprints, mapping and matcher provenance, decision summaries, policies, artifact names, and SHA-256 hashes.</p></div><button className="secondary" onClick={() => void downloadExport("manifest")} disabled={busy}>Download provenance manifest</button></article></div>{!run.trustedExportReadiness.ready && <div className="warning" role="status"><strong>Trusted export blocked.</strong><ul>{run.trustedExportReadiness.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul></div>}<div className="actions"><button className="secondary" onClick={() => setScreen("resolution")}>Review conflicts</button></div></section>}
...
function FilePicker({ side, file, onChange }: { side: "A" | "B"; file: File | null; onChange: (file: File | null) => void }) { return <label className={`file-picker${file ? " has-file" : ""}`}><span className="file-picker-heading"><span className="dataset-badge">{side}</span><strong>Dataset {side}</strong><small>{file ? "Selected" : "CSV required"}</small></span><span className="drop-target">{file ? <><strong className="selected-file">{file.name}</strong><small>Ready to upload · <span className="browse-affordance">choose another</span></small></> : <><strong>Drop CSV here</strong><small>or <span className="browse-affordance">browse files</span></small></>}</span><input aria-label={`Dataset ${side} CSV`} type="file" accept=".csv,text/csv" onChange={(event) => onChange(event.target.files?.[0] ?? null)} /></label>; }
