...
export function ReviewWorkspace({ run, initialCandidateId, busy, onDecision, onDefer, onUndo, onGoResolution }: ReviewWorkspaceProps) {
  ...
  return <section className="review-product" aria-labelledby="review-workspace-title">
    <header className="review-product-header"><div><p className="eyebrow">Step 5 · Human identity review</p><h1 id="review-workspace-title">Resolve identity uncertainty.</h1><p>Identity first. Values second. Scores are evidence scores, not probabilities.</p></div><div className="review-progress" aria-label="Overall review progress"><span><strong>{run.reviewProgress.reviewed}</strong> Reviewed</span><span><strong>{run.reviewProgress.remaining}</strong> Remaining</span><span><strong>{run.reviewProgress.deferred}</strong> Deferred</span><div aria-hidden="true"><i style={{ width: `${run.reviewProgress.total ? (run.reviewProgress.reviewed / run.reviewProgress.total) * 100 : 0}%` }} /></div></div></header>
    ...
    <div className="review-layout">
      ...
      <section className="review-case" aria-label="Selected identity case">
        ...
        {selectedCandidate && detail && !detailLoading && !detailError ? <>
          <header className="case-heading"><div><span className={`queue-state state-${selectedState}`}>{selectedState.replaceAll("_", " ")}</span><h2 ref={decisionHeadingRef} tabIndex={-1}>Are {selectedCandidate.aRowId} and {selectedCandidate.bRowId} the same entity?</h2></div><div className="score-block"><small>Match score</small><strong>{selectedCandidate.matchScore.toFixed(3)}</strong><span>Rank {selectedCandidate.rank} of {alternatives.length}</span></div></header>
          {(detail.collisionARowIds.length > 0 || selectedCandidate.collision) && <aside className="collision-warning" aria-label="Candidate collision warning"><strong>⚠ Shared candidate — inspect before confirming</strong><p>{selectedCandidate.bRowId} is also a candidate for {detail.collisionARowIds.join(", ") || "another A row"}.{detail.effectiveCollisionARowIds.length ? ` It is already an effective link for ${detail.effectiveCollisionARowIds.join(", ")}.` : " No competing effective link is being reassigned."}</p><div className="collision-links">{detail.collisionARowIds.map((aRowId) => { const competingItem = page?.items.find((item) => item.aRowId === aRowId); return competingItem ? <button type="button" key={aRowId} onClick={() => selectItem(competingItem)}>Inspect competing case {aRowId}</button> : null; })}</div><small>This run does not enforce global one-to-one identity. Samewise never overwrites another human decision automatically.</small></aside>}
          <section className="candidate-switcher" aria-labelledby="candidate-options-title"><header><h3 id="candidate-options-title">Candidate alternatives</h3><span>Selection alone never records a decision.</span></header><div role="list" aria-label="Dataset B candidate alternatives">{alternatives.map((candidate) => <CandidateOption key={candidate.candidateId} candidate={candidate} selected={candidate.candidateId === selectedCandidate.candidateId} onSelect={() => setSelectedCandidateId(candidate.candidateId)} />)}</div></section>
          <div className="evidence-hierarchy" aria-label="Evidence summary"><div className="positive"><small>Strongest evidence</small><strong>{[...selectedCandidate.evidence].sort((left, right) => right.positiveContribution - left.positiveContribution)[0]?.label ?? "No positive evidence"}</strong><span>{[...selectedCandidate.evidence].sort((left, right) => right.positiveContribution - left.positiveContribution)[0]?.explanation}</span></div><div className="negative"><small>Potential conflict</small><strong>{[...selectedCandidate.evidence].sort((left, right) => right.conflictContribution - left.conflictContribution)[0]?.conflictContribution ? [...selectedCandidate.evidence].sort((left, right) => right.conflictContribution - left.conflictContribution)[0]?.label : "No weighted contradiction"}</strong><span>{selectedCandidate.strongContradiction ? "Strong identifier contradiction blocks auto-match." : "Review all fields before deciding."}</span></div></div>
          <section className="entity-comparison" aria-labelledby="field-evidence-title"><header><h3 id="field-evidence-title">Mapped identity evidence</h3><span>Dataset A raw value</span><span>Dataset B raw value</span></header>{selectedCandidate.evidence.map((evidence) => <article className={`field-evidence evidence-${evidence.evidenceClass}`} key={evidence.mappingId}><div><strong>{evidence.label}</strong><span className="evidence-state">{evidenceStateLabel(selectedCandidate, evidence.mappingId)}</span><small>{evidence.fieldKind} · contribution {evidence.contribution >= 0 ? "+" : ""}{evidence.contribution.toFixed(3)}</small></div><div><span className="dataset-label dataset-a">Dataset A</span><strong>{evidence.aValue || "— Missing"}</strong>{evidence.normalizedA && evidence.normalizedA !== evidence.aValue && <small>Normalized: {evidence.normalizedA}</small>}</div><div><span className="dataset-label dataset-b">Dataset B</span><strong>{evidence.bValue || "— Missing"}</strong>{evidence.normalizedB && evidence.normalizedB !== evidence.bValue && <small>Normalized: {evidence.normalizedB}</small>}</div><details><summary>Feature detail and matcher explanation</summary><p>{evidence.explanation}</p><dl><div><dt>Weight</dt><dd>{evidence.weight.toFixed(3)}</dd></div><div><dt>Positive</dt><dd>{evidence.positiveContribution.toFixed(3)}</dd></div><div><dt>Conflict</dt><dd>{evidence.conflictContribution.toFixed(3)}</dd></div>{evidence.features.map((feature) => <div key={feature.name}><dt>{feature.name.replaceAll("_", " ")}</dt><dd>{feature.value.toFixed(3)}</dd></div>)}</dl></details></article>)}</section>
          ...
          <footer className="review-actions" aria-label="Identity review actions"><div><span>Matcher {run.matcherVersion}</span><button type="button" className="shortcut-help" aria-label="Keyboard shortcuts: S same, D different, E defer, U undo, J and K navigate, 1 through 3 select candidates">Shortcuts <kbd>S</kbd> <kbd>D</kbd> <kbd>E</kbd> <kbd>U</kbd> <kbd>J/K</kbd> <kbd>1–3</kbd></button></div>{deferred ? <button type="button" className="secondary" onClick={() => void deferCurrent(false)} disabled={busy}>Return to review</button> : <button type="button" className="secondary" onClick={() => void deferCurrent(true)} disabled={busy || selectedState !== "needs_review"}>Defer / skip <kbd>E</kbd></button>}<button type="button" className="different" aria-label="Different entity" onClick={() => void decide("different_entity")} disabled={busy || !!candidateDecision || selectedState === "reviewed_same" || selectedState === "auto_match"}>Different entity <kbd>D</kbd></button><button type="button" className="same" aria-label="Same entity" onClick={() => void decide("same_entity")} disabled={busy || !!candidateDecision || selectedState === "reviewed_same" || selectedState === "auto_match"}>Same entity <kbd>S</kbd></button></footer>
          {detail.conflicts.length > 0 && <div className="post-identity-conflicts"><span><strong>{detail.conflicts.length} field conflict{detail.conflicts.length === 1 ? "" : "s"}</strong> created after SAME. Zero values were selected automatically.</span><button type="button" onClick={onGoResolution}>Resolve values separately</button></div>}
        </> : !detailLoading && !detailError && <div className="review-complete"><h2>No active review case</h2><p>Change the queue filter or page to inspect another item.</p></div>}
      </section>
    </div>
  </section>;
}
